...
if (page === "home-page") {
    document.addEventListener("DOMContentLoaded", () => {
        ...
        checkButton.textContent = "Check Best Parking Level";
        checkButton.className = "btn btn-secondary btn-lg";
        ...
        heroText.appendChild(checkButton);
        heroText.appendChild(result);

        checkButton.addEventListener("click", () => {
            const availableNumbers = document.querySelectorAll(".available-num");
            let bestLevel = 1;
            let mostAvailable = 0;

            availableNumbers.forEach((cell, index) => {
                const available = parseInt(cell.textContent);

                if (available > mostAvailable) {
                    mostAvailable = available;
                    bestLevel = index + 1;
                }
            });

            result.textContent = `Best option: Level ${bestLevel} has ${mostAvailable} available spots.`;
        });
    });
}

if (page === "simulation-page") {
    document.addEventListener("DOMContentLoaded", () => {
        const spotToggles = document.querySelectorAll(".spot-toggle");
        ...
        function updateSimulationTotals() {
            ...
            document.querySelectorAll(".level-card").forEach((levelCard) => {
                const levelSpots = levelCard.querySelectorAll(".spot-toggle");

                let levelAvailable = 0;
                let levelOccupied = 0;
                let levelLeaving = 0;

                levelSpots.forEach((spot) => {
                    if (spot.checked) {
                        levelOccupied++;
                    } else if (spot.classList.contains("spot-toggle--leaving")) {
                        levelLeaving++;
                    } else {
                        levelAvailable++;
                    }
                ...
                totalLeaving += levelLeaving;

                const greenCount = levelCard.querySelector(".count-green");
                const redCount = levelCard.querySelector(".count-red");
                const yellowCount = levelCard.querySelector(".count-yellow");
                const grayBadge = levelCard.querySelector(".badge-gray");
                const statusBadge = levelCard.querySelector(".level-card__meta .badge:first-child");

                if (greenCount) greenCount.textContent = levelAvailable;
                if (redCount) redCount.textContent = levelOccupied;
                if (yellowCount) yellowCount.textContent = levelLeaving;
                if (grayBadge) grayBadge.textContent = `${levelAvailable} / ${levelSpots.length} available`;

                if (statusBadge) {
                    if (levelAvailable === 0) {
                        statusBadge.textContent = "Full";
                        statusBadge.className = "badge badge-red";
                    } else if (levelAvailable <= 5) {
            ...
            const topNumbers = document.querySelectorAll(".control-stat__num");

            if (topNumbers[0]) topNumbers[0].textContent = totalAvailable;
            if (topNumbers[1]) topNumbers[1].textContent = totalOccupied;
            if (topNumbers[2]) topNumbers[2].textContent = totalLeaving;
        }

        spotToggles.forEach((toggle) => {
            toggle.addEventListener("change", () => {
                const label = document.querySelector(`label[for="${toggle.id}"]`);
                if (!label) return;

                const statusText = label.querySelector(".spot__status");
                if (!statusText) return;

                if (toggle.checked) {
                    statusText.textContent = "Taken";
                } else if (toggle.classList.contains("spot-toggle--leaving")) {
